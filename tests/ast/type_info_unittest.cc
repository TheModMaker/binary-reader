...
TEST(IntegerTypeInfoTest, ReadValue_UnsignedInt) {
  ...
  IntegerTypeInfo integer("", 32, Signedness::Unsigned, ByteOrder::BigEndian);
  ASSERT_TRUE(
...
TEST(IntegerTypeInfoTest, ReadValue_LargeUnaligned) {
  ...
  Value result;
  ErrorCollection errors("");
  IntegerTypeInfo int32("", 32, Signedness::Unsigned, ByteOrder::BigEndian);
  ASSERT_TRUE(int32.ReadValue(
      MakeReader({0x12, 0x34, 0x56, 0x78, 0x90, 0x12}, kOffset).get(), &result,
      &errors));
  ASSERT_EQ(result, Value{0x23456789u});

  IntegerTypeInfo int64("", 64, Signedness::Unsigned, ByteOrder::BigEndian);
...
TEST(IntegerTypeInfoTest, ReadValue_SmallUnaligned) {
  ...
  Value result;
  ErrorCollection errors("");
  IntegerTypeInfo integer("", 5, Signedness::Unsigned, ByteOrder::BigEndian);
  // 0110 1011
  ASSERT_TRUE(
...
TEST(IntegerTypeInfoTest, ReadValue_SignedInt) {
  Value result;
  ErrorCollection errors("");
  IntegerTypeInfo integer("", 32, Signedness::Signed, ByteOrder::BigEndian);
  ASSERT_TRUE(
      integer.ReadValue(MakeReader({0x11, 0x22, 0x33, 0x44, 0x55, 0x66}).get(),
                        &result, &errors));
...
TEST(IntegerTypeInfoTest, ReadValue_Negative) {
  Value result;
  ErrorCollection errors("");
  IntegerTypeInfo int16("", 16, Signedness::Signed, ByteOrder::BigEndian);
  ASSERT_TRUE(
      int16.ReadValue(MakeReader({0xff, 0xcd, 0x0}).get(), &result, &errors));
  ASSERT_EQ(result, Value{-51});
  IntegerTypeInfo int64("", 64, Signedness::Signed, ByteOrder::BigEndian);
  ASSERT_TRUE(int64.ReadValue(
      MakeReader({0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xcd, 0x0}).get(),
      &result, &errors));
  ASSERT_EQ(result, Value{-51});
  IntegerTypeInfo int_little("", 16, Signedness::Signed,
